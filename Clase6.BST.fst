...
let rec all_lt_not_in (x:int) (t:bst0) (y:int)
  : Lemma (requires all_lt y t /\ x >= y)
          (ensures in_tree x t = false)
= match t with
  | L -> ()
  | N (l, z, r) -> 
    all_lt_not_in x l y;
...
let rec all_gt_not_in (x:int) (t:bst0) (y:int)
  : Lemma (requires all_gt y t /\ x <= y)
          (ensures in_tree x t = false)
= match t with
  | L -> ()
...
let rec member_ok (x:int) (t:bst) : Lemma (member x t == in_tree x t) =
  ...
  | N (l, y, r) ->
    if x = y then ()
    else if x < y then (all_gt_not_in x r y;
                        member_ok x l)
    else (all_lt_not_in x l y;
          member_ok x r)
...
= match t with
  | N (L, z, r) -> ()
  | N (l, z, r) -> extract_min_preserva_all_lt y l

let rec extract_min_preserva_all_gt (y:int) (t : bst0{N? t})
  : Lemma (requires is_bst t /\ all_gt y t)
          (ensures all_gt y (snd (extract_min0 t)))
= match t with
  | N (L, z, r) -> ()
  | N (l, z, r) -> extract_min_preserva_all_gt y l
